...
public class CS1001 {
	public CS1001() {
		...
		String MyName = GAutoName.getAutoName();
		String MyFemaleEnglishName = GAutoName.getAutoFemaleEnglishName();
		String MyMaleEnglishName = GAutoName.getAutoMaleEnglishName();
		String MyEnglishName = GAutoName.getAutoEnglishName();
		String MyAutoName = GAutoName.AutoName();
		...
		try {
			...
			/* 必填项，用于记录发送报文或者另外自己编写的输入参数表 */
			GTestCase.RecordInputParams(GParam.gReq, "CS1001");

			// 如果名字不等于预设值则认为名字获取成功，总数加1;只取到一个名字时为有效，其他情况则抛出异常，没有获取到抛出“失败”，获取到1个以上名字抛出计划内“异常”
			...
			case 0: {
				...
				GParam.TestResultMsg = "既然没有名字，那就给你一个：" + GAutoName.AutoName();
				break;
			}
			case 1: {
