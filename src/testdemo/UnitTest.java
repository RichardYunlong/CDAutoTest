...
public class UnitTest {
    ...
    public static void testGetNetworkIP() {
    	GLog.logRecord(8, GRemote.getNetworkIP(GTransfer.getNetworkSegment()));
    }
	...
	public static void testShowClassParams(String clazzName) {
		try {
			...
			Field[] fields = onwClass.getClass().getDeclaredFields();
    ...
    public static void testSetUrlContent() {
    	GLog.logRecord(8, "" + GJsonObjectMapper.writeUrlJson("./json/aqi-login2desktop.json", "http://10.0.104.47:28080/echarts-website-asf-site/data/aqi-login2desktop.json"));	
    }
	/**
	 *  测试获得中文姓名 
	 */
	...
	protected static void testGDragonII(){
		new GDragonII(GAbout.getWelcome(), 0, 0);
	}
	...
	protected static void testGLogShear() {
    	for(int i = 0;i < 10;i++) {
    		GLog.logShear("./test/log_sys.txt", 20000);
    	}
    }
	...
	protected static void testGTestCaseCounts(){
		...
		GTestCaseCounts.setTestCaseCounts(testCaseCounts);
		
		GTestCaseCounts.exportReport();
		try {
			Runtime.getRuntime().exec(GMsg.MSG_CMD[0] + GTestCaseCounts.getTESTCASECOUNT_NAME());
		} catch (IOException e) {
	...
	protected static void testGDetail(){
		...
		try {
			Runtime.getRuntime().exec(GMsg.MSG_CMD[0] + GDetail.getREPORT_NAME());
		} catch (IOException e) {
	...
	protected static void testGSubstitute(){
		...
		try {
			Runtime.getRuntime().exec(GMsg.MSG_CMD[0] + GSubstitute.getSUBSTITUTE_NAME());
		} catch (IOException e) {
			GLog.logSysFunctionException("testGSubstitute", e);
		}
	}
	...
	protected static void testGMsg(){
		GAbout.setDefault();
		GLog.logRecord(8, GAbout.getWelcome());	
		GLog.logRecord(8, GAbout.getEnding());
	}
	...
	protected static void testGObject2dimension() {
	    ...
	    if(GObject2dimension.getInputList() == null) {
	    	...
	    	GLog.logRecord(8, content + Arrays.toString(GObject2dimension.getInputList()) + contentNum + String.valueOf(GObject2dimension.getTotalNum()));
	    }
