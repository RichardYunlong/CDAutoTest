...
public class GWCtrlPage {
	...
	public static void ui_D_IFRAME_INDEX(WebDriver webDriver, int tabIndex, String waitByType, String waitByTar) {
		...
		try {
			if (GWCtrlWebElementIframe.getIframe(tabIndex) != null) {
				...
				GLog.logRecordTime(9, "----<iframe<waitByType[" + waitByType + "];waitByTar[" + waitByTar + "]>>"
						+ GWCtrlMsg.ui_QUERY[1]);
			}
			GWCtrlFrame.ui_C_SWITCN_DEFAULT(webDriver);
		} catch (Exception e) {
			GWCtrlException.switchTo(webDriver, e, 1, 0,
					"----<iframe<waitByType[" + waitByType + "];waitByTar[" + waitByTar + "]>>" + GWCtrlMsg.ui_QUERY[2],
					true);
		}
	 ...
	 */
	public static void ui_C_CLICK_TAB(WebDriver webDriver, String iframeId, String iframeXpath, String tagName,
			String tabPropertyName, String tabPropertyValue, String tabText) {
		...
		try {
			...
			WebElement tab = GWCtrlStaticFind.getWebElementByIdOrXpath(webDriver, iframeId, "/html/body", tagName,
					tabPropertyName, tabPropertyValue, tabText);
			...
			GWCtrlException.switchTo(webDriver, e, 1, 0,
					"----<exception[tab[" + tabText + "] " + GWCtrlMsg.ui_CLICK[1] + "]>", true);
		}
	...
	public static void ui_C_CLICK_INPUT_BTN(WebDriver webDriver, String rootId, String rootXpath, String tabName) {
		...
		try {
			...
			GWCtrlException.switchTo(webDriver, e, 1, 0,
					"----<exception[inputbutton[" + rootId + "]" + GWCtrlMsg.ui_QUERY[2] + "]>", true);
		}
	...
	public static void ui_C_SELECT_DATE_STR(WebDriver webDriver, String date) {
		...
		String dateDivRootSelector = GText.getCssSelectorTxt("div", "class",
				"x-menu x-menu-floating x-layer x-date-menu x-menu-plain");
	...
	public static void ui_C_SELECT_DATE(WebDriver webDriver, String date) {
		...
		try {
			for (int i = 10; i <= 20; i++) {
				...
				WebElement webDateDiv = GWCtrlStaticFind.getWebElementByIdOrXpath(webDriver, "", strDateDiv, "button",
						date);
				if (webDateDiv != null) {
					webDateDiv.click();
	...
	public static void ui_C_SELECT_COOPRATION(WebDriver webDriver, String cooTable, String cooTar) {
		...
		try {
			...
			GWCtrlException.switchTo(webDriver, e, 1, 0,
					"----<exception[table[" + cooTable + "];cooTar[" + cooTar + "]" + GWCtrlMsg.ui_QUERY[2] + "]>",
					true);
		}
	...
	public static boolean ui_C_SEARCH_COO(WebDriver webDriver, String cooName) {
		...
		if (layoutPanel1 != null) {
			try {
				List<WebElement> colCodes = layoutPanel1.findElements(By.cssSelector(GText.getCssSelectorTxt("div",
					...
					for (WebElement colCode : colCodes) {
						...
						for (WebElement input : inputs) {
							if (input.getAttribute("type").equals("text")) {
								...
								GWCtrlWait.ViewWaitingTextByWebElement(webDriver, GTestIndicators.PageShowTime,
										layoutPanel1, cooName);
								WebElement coo;
								if (ui_C_FIND_WEBELEMENT_EXIST(webDriver, GText.getCssSelectorTxt("div", "class",
										"x-grid3-cell-inner x-grid3-col-0 x-unselectable"))) {
									// 适用于无勾选框场景
									coo = webDriver.findElement(By.cssSelector(GText.getCssSelectorTxt("div", "class",
											"x-grid3-cell-inner x-grid3-col-0 x-unselectable")));
								} else {
									// 适用于有勾选框场景
									coo = webDriver.findElement(By.cssSelector(GText.getCssSelectorTxt("div", "class",
											"x-grid3-cell-inner x-grid3-col-1 x-unselectable")));
								}
								...
								for (WebElement span : spans) {
									if (span.getText().equals(cooName)) {
										...
										GLog.logRecordTime(9,
												"----<table<span[" + cooName + "]>>" + GWCtrlMsg.ui_CLICK[0]);
										break;
									}
								}
							}
				...
				GWCtrlException.switchTo(webDriver, e, 1, 0,
						"----<exception[table[" + cooName + "]" + GWCtrlMsg.ui_QUERY[2] + "]>", true);
			}
		}
		GLog.logRecordTime(9, "]]----[table]----[widget]");
		return isClick;
	 ...
	 */
	public static void ui_C_SELECT_JIAFANG(WebDriver webDriver, String jiafangSearch, String jiafangTable,
			String jiafangTar) {
		GLog.logRecordTime(9, "[widget]----[table]----[[");
		try {
			...
			GWCtrlWait.ViewWaitingAllByCssSelector(webDriver, GTestIndicators.PageShowTime,
					GText.getCssSelectorTxt("div", "class", "x-grid3-cell-inner x-grid3-col-0 x-unselectable"));
			...
			GWCtrlException.switchTo(webDriver, e, 1, 0,
					"----<exception[table[" + jiafangTable + "];tar[" + jiafangTar + "]" + GWCtrlMsg.ui_QUERY[2] + "]>",
					true);
		}
		GLog.logRecordTime(9, "]]----[table]----[widget]");
	}
	...
	public static void ui_C_CLICK_VERIFY(WebDriver webDriver, String eId, String sName, boolean bOK, String wId) {
		...
		try {
						...
						if (sName.equals("删除") && eId.equals("btnDeleteYS")) {// 如果点击的是【施工合同登记-预算书-删除】
							...
							for (WebElement buttonS2 : buttonsS2) {
								if (button.getText().equals("否")) {
									buttonS2.click();
									GLog.logRecordTime(9,
											"----<verifywindow[" + button.getText() + "]>" + GWCtrlMsg.ui_CLICK[0]);
								}
							}
						}
						break;
			...
			GWCtrlException.switchTo(webDriver, e, 1, 0,
					"----<exception[verifywindow[" + eId + "]]>" + GWCtrlMsg.ui_CLICK[1], true);
		}
	 ...
	 * @param wId       等待元素Id
	 */
	public static void ui_C_CLICK_VERIFY(WebDriver webDriver, int tabIndex, String eId, String sName, boolean bOK,
			String wId) {
		GLog.logRecordTime(9, "[widget]----[verifywindow]----[[");
		...
		try {
						...
						if (sName.equals("删除") && eId.equals("btnDeleteYS")) {// 如果点击的是【施工合同登记-预算书-删除】
							...
							for (WebElement buttonS2 : buttonsS2) {
								if (button.getText().equals("否")) {
									buttonS2.click();
									GLog.logRecordTime(9,
											"----<verifywindow[" + buttonS2.getText() + "]>" + GWCtrlMsg.ui_CLICK[0]);
								}
							}
						}
						break;
			...
			if (sName.equals("删除")) {
				if (!wId.isEmpty()) {
					if (wId.equals(GWCtrlWebElementId.CN_ID.get("单据列表"))
							|| wId.equals(GWCtrlWebElementId.CN_ID.get("字典列表"))) {
	 ...
	 * @param wId       等待元素id
	 */
	public static void ui_C_CLICK_ID_VERIFY(WebDriver webDriver, int tabIndex, String btnId, String btnName,
			boolean btnChoose, String wId) {
	...
	public static void ui_C_CLICK_BLANK_SPACE(WebDriver webDriver, String atrributeName, String atrributeValue) {
		...
		try {
			...
			GLog.logRecordTime(9,
					"----<blank_Space[" + Objects.requireNonNull(blankSpace) + "]>" + GWCtrlMsg.ui_CLICK[0]);
		} catch (Exception e) {
	...
	private static void ui_C_WAIT_DESKTOP(WebDriver webDriver) {
		try {
			...
			GWCtrlWait.ViewWaitingAllById(webDriver, GTestIndicators.DesktopShowTime,
					GWCtrlWebElementId.CN_ID.get("desktop"));
			GWCtrlFrame.ui_C_SWITCN_DEFAULT(webDriver);
			GLog.logRecordTime(9,
					"----<webelement[" + GWCtrlWebElementId.CN_ID.get("desktop") + "]>" + GWCtrlMsg.ui_QUERY[1]);
		} catch (NoSuchElementException e) {
			GLog.logRecordTime(9,
					"----<webelement[" + GWCtrlWebElementId.CN_ID.get("desktop") + "]>" + GWCtrlMsg.ui_QUERY[2]);
		}
	}
}
