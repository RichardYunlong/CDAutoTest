...
public class GTracker {
	...
	public void saveSpendTimesJsonDataByName(String jsonPath, String jsonSplit, String tc_name) {
		int testCaseTimes;
		String version = "[" + GTime.getCurrentTime(GTime.FORMAT_14_TEXT) + "][" + GStatic.gSut.getSysName() + "+"
				+ GStatic.gSut.getSectionName1() + "+" + GStatic.gSut.getSectionName2() + "+"
				+ GStatic.gSut.getSectionName3() + "][" + tc_name + "]";
		GLog.logRecord(7, "动作名称:" + tc_name);
		if (this.trackerSpendTimeTree.get(tc_name) != null) {
			if (this.trackerSpendTimeTree.get(tc_name).getSpendTimes() != null) {
				testCaseTimes = this.trackerSpendTimeTree.get(tc_name).getSpendTimes().size();
				if (testCaseTimes > 0) {
					this.trackerAverageSpendTimeTree.put(tc_name + jsonSplit + this.trackerName,
							this.trackerSpendTimeTree.get(tc_name).logAverageSpendTime());
					GFile.writeStringToRight(jsonPath,
							"[\"" + version + "\", "
									+ this.trackerSpendTimeTree.get(tc_name).getAverageSpendTime().toString() + "]"
									+ jsonSplit);
				}
			}
		}
		...
		if (this.trackerSpendTimeTree.get(tc_name) != null) {
			if (this.trackerSpendTimeTree.get(tc_name).getSpendTimes() != null) {
				...
				if (testCaseTimes > 0) {
					...
					this.trackerAverageSpendTimeTree.put(moduleTemp,
							this.trackerSpendTimeTree.get(tc_name).logAverageSpendTime());
					res = version + "," + this.trackerSpendTimeTree.get(tc_name).getAverageSpendTime().toString() + "]"
							+ jsonSplit;
				}
			}
		}
