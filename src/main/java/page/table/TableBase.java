...
public class TableBase extends UniqueWebElementBase {
	...
	public TableBase(WebDriver webDriver,
		...
		if (null == super.getUniqueRoot()) {
			gridRoot = GWCtrlQuery.ui_Q(webDriver, "cssSelector",
					GText.getCssSelectorTxt(locateTagName, locateAtrributeName, locateArributeValue));
			if (null != gridRoot) {
		...
		try {
			gridVerticalScrollbar = Objects.requireNonNull(gridRoot).findElement(By.cssSelector(
					GText.getCssSelectorTxt(locateTagName, locateAtrributeName, locateArributeValueVerticalScrollbar)));
		...
		try {
			gridHorizontalScrollbar = gridRoot.findElement(By.cssSelector(GText.getCssSelectorTxt(locateTagName,
					locateAtrributeName, locateArributeValueHorizontalScrollbar)));
	...
	public void initRows(WebDriver webDriver,
			...
			String locateArributeValue) {
		rows = new ArrayList<>();
		List<WebElement> rowsTemp = new ArrayList<>(
				GWCtrlQuery.findElements(webDriver, table, locateTagName, locateAtrributeName, locateArributeValue));
		if (!rowsTemp.isEmpty()) {
