...
public class TenantDropDown {
	...
	public void click(WebDriver webDriver) {
		if (!isSame) {
			...
			if (null != menuitem) {
				...
				List<WebElement> menuItems = menuitem
						.findElements(By.cssSelector(GParam.getCssSelectorBy3K("租户列表_目标")));
				...
				try {
					GWCtrlWait.ViewWaitingAllByCssSelectorNotEffect(webDriver, 4,
							GParam.getCssSelectorBy3K("租户名称_确认窗体"));
		...
		if (!isSame) {
			if (null != confirmWindow) {
				...
				GLog.logRecordTime(9,
						confirmWindow.findElement(By.cssSelector(GParam.getCssSelectorBy3K("租户名称_确认提示"))).getText());

				List<WebElement> buttons = confirmWindow
						.findElements(By.cssSelector(GParam.getCssSelectorBy3K("租户名称_确认按钮")));
	...
	public TenantDropDown(WebDriver webDriver, String name) {
		try {
			...
			GWCtrlException.switchTo(webDriver, e, 1, 0, "----<exception[TenantDropDown" + GWCtrlMsg.ui_QUERY[2] + "]",
					true);
		}
	}
}
