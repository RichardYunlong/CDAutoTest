...
public class QueryScheme extends UniqueWebElementBase {
	...
	public void input(WebDriver webDriver, String name, String value) {
		if (null != queryCriteriasInputType) {
			...
			GWCtrlWait.ViewWaitingAllByCssSelector(webDriver, GTestIndicators.PageShowTime,
					GParam.getCssSelectorBy3K("筛选条件_右侧"));
		}
	}
	...
	public void setting(WebDriver webDriver, String name, String value) {
		...
		try {
			...
			filter = webDriver
					.findElement(By.cssSelector(GText.getCssSelectorTxt("input", "fieldid", filterTagProValue)));
		} catch (Exception e) {
	...
	public void click(WebDriver webDriver, String buttonName) {
		...
		try {
			...
			GWCtrlWait.ViewWaitingAllByCssSelector(webDriver, GTestIndicators.PageShowTime,
					GText.getCssSelectorTxt("button", "fieldid", buttonTagProValue));
			button = webDriver
					.findElement(By.cssSelector(GText.getCssSelectorTxt("button", "fieldid", buttonTagProValue)));
		} catch (Exception e) {
			GWCtrlException.switchTo(webDriver, e, 1, 0, "----<exception[" + buttonName + GWCtrlMsg.ui_QUERY[2] + "]",
					true);
		}
		if (null != button) {
			button.click();
	...
	public void reload(WebDriver webDriver) {
		GWCtrlWait.ViewWaitingAllByCssSelector(webDriver, GTestIndicators.PageShowTime,
				GText.getCssSelectorTxt("div", "class", "new-filter-container"));
		querySchemeRoot = webDriver
				.findElement(By.cssSelector(GText.getCssSelectorTxt("div", "class", "new-filter-container")));
	}
}
