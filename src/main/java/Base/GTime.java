...
public class GTime {
	...
	public static int getCurrentTime14Split(int index) {
		...
		for(int i = 0;i < dTimeSplit.length;i++) {
			dTimeSplit[i] = Integer.parseInt(strTimeSplit[i]);
		}
    ...
    public static String getAccountingPeriod(String type, String region, String format) {
        ...
        switch (type) {
            case "year":
                ...
                break;
            case "month":
                int dMonth = Integer.parseInt(strTimeSplit[1]);
                ...
                break;
            case "day":
                if (region.equals(startCsr) || region.equals("end")) {
	...
	public static void pause(int stime) {
		try {
			...
			GFile.writeStringErrorToGuideBottom("pause[" + Arrays.toString(e.getStackTrace()) +"]");
		}
	}
}
