...
public class GText {
	...
	public static void deteleTag(String filePath, String tag) {
		...
		try(BufferedReader br = new BufferedReader(new FileReader(file));
			...
			GFile.writeStringToBottom(GSys.GUIDE,"DELETE ROW WHICH CONTAIN[" + tag + "] OK!");
		} catch (Exception e) {
	...
	public static String getRandomString(int length, String letterType) {
		...
		try {
			...
			GFile.writeStringErrorToGuideBottom(GMsg.MSG_RANDOM[0] + "[" + Arrays.toString(e.getStackTrace()) +"]");
		}
		...
		try (RandomAccessFile accessFile = new RandomAccessFile(fin, "r");
			...
			while (fcin.read(rBuffer) != -1) {
				...
				if (rSize > tempString.length()) {
					String substring = tempString.substring(fromIndex);
					strline.append(substring);
					strBuf.append(substring);
				} else {
