...
public class GException {
	...
	public static void doCatchException(String strReq, Exception e) {
		if (!GParam.getRes().equals("") && (GTestCase.getTCTypeByNo(GTestCase.getTC_NO()) == 2)) {
			GLog.logRecord(9, "UNKNOW ERROR,FROM:" + strReq);
			GTestCase.recordTCResult(2);
		} else {
			GTestCase.recordTCResult(1);
			GSys.logErrorSys("doCatchException[" + Arrays.toString(e.getStackTrace()) +"]");
		}
	}
	...
	public static void doCatchIOException(String strReq, IOException e) {
		if (!GParam.getRes().equals("") && (GTestCase.getTCTypeByNo(GTestCase.getTC_NO()) == 2)) {
			...
			GTestCase.recordTCResult(1);
			GSys.logErrorSys("doCatchIOException[" + Arrays.toString(e.getStackTrace()) +"]");
		}
	}
	...
	public static boolean SwtichTo (Exception e, int dTSStyle, int logIndex, String eMsg) {
		...
		try {
			...
			//用例类型判断
			if (GTestCase.getTC_TYPE_RES() == 1) {
				//如果预制的用例类型就是1，即失败用例，将错误堆栈信息作为返回信息
				GParam.setRes(GException.getExceptionAllinformation(e));
			}else {
				//如果预制的用例类型不是1，即测试用例执行失败是计划外的，需要将测试用例类型置为1，即转变为失败用例，供用例执行完成后的数量变更
				GTestCase.setTC_TYPE_RES(Integer.valueOf(1));
			}
